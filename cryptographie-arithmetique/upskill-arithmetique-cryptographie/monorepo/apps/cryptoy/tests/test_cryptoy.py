...
from cryptoy.utils import binary_strings_to_bytes, int_to_binary, split_binary_strings


def test_cryptoy_caesar_cipher() -> None:
...
def test_cryptoy_attack_caesar_cipher() -> None:
...
def test_cryptoy_affine_cipher_optimized() -> None:
    key = 13
...
def test_cryptoy_attack_affine_cipher() -> None:
...
def test_cryptoy_attack_affine_cipher_optimized() -> None:
...
def test_cryptoy_passwords_attack() -> None:
...
def test_cryptoy_passwords_fix() -> None:
    passwords_list, passwords_database = load_passwords_data()
...
def test_cryptoy_diffie_hellman() -> None:
    prime_number = [SUPPRIME_2600]0278034973198552060607533234122603254684088120031105907484281003994966956119696956248629032338072839127039
    generator = 2
...
def test_cryptoy_rsa_decrypt() -> None:
    the_key = {
        "public_key": (
            65537,
    ...
    cipher_text = [SUPPRIME_2600]
