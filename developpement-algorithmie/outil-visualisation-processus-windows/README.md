...
Notes d'implémentation et suggestions
-------------------------------------
- Commencez par implémenter une liste basique des processus (PID, nom) avant d’ajouter les colonnes détaillées (mémoire, CPU, handles, etc.).
- Implémentez une option `-d <PID>` pour afficher les threads d’un processus donné et leurs informations détaillées.
- Prévoyez des options pour trier (par PID, par nom, par usage CPU) et pour filtrer par nom.
- Documentez clairement les permissions requises (certaines informations peuvent nécessiter des droits administrateur).

Voulez-vous que je génère un `README.txt` additionnel avec un exemple de commandes `cl / gcc` / instructions de compilation Windows et un modèle de `plist.exe` minimal ?
