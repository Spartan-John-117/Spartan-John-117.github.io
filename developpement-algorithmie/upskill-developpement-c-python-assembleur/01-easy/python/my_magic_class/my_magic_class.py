...
class Dummy:
    def __init__(self, *args, **kwargs):
        self._args = list(args)                             #On crée une liste contenant les arguments positionel
        self.desc = kwargs.get("desc", "instance magic")    #desc a pour valeur par défaut "instance magic"
        
        for key, value in kwargs.items():
            setattr(self, key, value)                       #On prend tous les arguments nommés et on en fait des attribus
    
    def __getitem__(self, index):
        return self._args[index]                            #On parcours la liste des args et on retourne chaque argument
    
    def __len__(self):
        return len(self._args)                              #On retourne la longueur des arguments positionnels
