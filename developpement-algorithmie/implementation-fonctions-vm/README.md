...
Chaque processeur (réel ou virtuel) manipule un nombre limité de registres. Ici :

- Registres génériques : A, B, C (utilisés pour calculs).
- Registres spécifiques : ex. `sp` (stack pointer).

La pile système est utilisée via les méthodes `push` et `pop` fournies dans la classe `Machine` :

- La pile fait une taille de 100 fois la taille d’un registre.
- Une opération `PUSH` stocke la valeur d'un registre générique sur la pile et décrémente `sp`.
- Une opération `POP` lit la valeur au sommet de la pile et l’affecte au registre associé, puis incrémente `sp`.
- Quand `sp` atteint 0, la pile est pleine. Quand `sp` atteint 100, la pile est vide. Toute opération sortant du domaine 0-100 provoque une exception Python stoppant la machine.


### À implémenter (interprète)

Vous devrez compléter l'interprète dans les parties `TODO` pour les instructions suivantes :

- `ADD`: A := B + C
- `MUL`: A := B * C
- `PUSHB`, `PUSHC`: prendre la valeur du registre B (ou C) et la stocker sur la pile via `push`.
- `POPB`, `POPC`: utiliser `pop` pour récupérer la valeur et la stocker dans B (ou C).
...
- ajouter un exemple minimal de fichier `vm.py` illustrant la structure de la classe `Machine` et le dictionnaire `instructions` ;
- générer des tests unitaires pour vérifier `ADD`, `MUL`, `PUSH`/`POP` et la gestion de débordement de pile ;
- ou encore formater et intégrer les fichiers de l'exercice existants dans ce dossier.
