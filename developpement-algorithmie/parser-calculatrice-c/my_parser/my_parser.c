...
int                     readtext(struct parser *p, char *text)
{
    int tmp = p->current_pos;
    int i = 0;
    while (text[i] && readchar(p, text[i]))
        i += 1;
    if (text[i] == 0)
        return 1;
    reset_pos(p, tmp);
...
int              readfloat_dec(struct parser *p)
{
    ...
    {
        (void) (readint(p));    // void indique une partie optionnelle correspondant au "Int?"
        return 1;
    }
    reset_pos(p, tmp);
    return 0;
...
int              readfloat_exp(struct parser *p)
{
    ...
    {
        (void)(readchar(p, '-') || readchar(p, '+'));   // void indique une partie optionnelle correspondant au "('-' / '+')?"
        if (readint(p))
            return 1;
    }
    reset_pos(p, tmp);
...
int                     readfloat(struct parser *p)
{
    ...
    {
        (void)(readfloat_exp(p));   // void indique une partie optionnelle correspondant au "Exp?"
        return 1;
    }
    reset_pos(p, tmp);
    return 0;
